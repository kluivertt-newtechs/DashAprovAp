...
import { PoDialogService, PoModule, PoNotificationService, PoPageModule, PoTableColumn } from '@po-ui/ng-components';
...
export class HomeComponent implements OnInit, OnDestroy{
      ...
      next: result => {
        ...
        if (this.itensDash.length > 0) {
          ...
          this.columnsDash = keys
            .filter(key => key !== 'empresa' && key !== 'totalEmpresa' && key !== 'chaveDetalhe')
            .map(key => ({
              property: key,
  ...
  ImprimeDados() {
    this.escondeTimer = false
    this.homeService.getRelat(this.filtros).subscribe(
      resposta => {
